...
- **Metric collection** — **write the goldens first, then pick metrics the goldens can actually score.** A metric whose required field is missing from even one golden errors on that test case, so the user's first run comes back red through no fault of their own. Pick **3–6**, `name` strictly from the lists below (anything else is dropped server-side). Use the multi-turn list with `"multiTurn": true` for conversation-loop apps, the single-turn list otherwise. `threshold` (0–1) is optional.
  - Single-turn, **always safe** (need only the input and the app's answer): Answer Relevancy, Bias, PII Leakage, Summarization, Toxicity. Image apps only: Image Coherence, Image Editing, Image Helpfulness, Image Reference.
  - Single-turn, **conditional** — only if **every** golden carries the field:
  ...
  - Single-turn, **trace-only** — these score off a captured trace, so they stay empty until the tracing PR is merged and traces arrive; they produce nothing on a dataset run. Agent apps only, and at most one: Task Completion, Step Efficiency, Plan Adherence, Plan Quality.
  - Multi-turn: Conversation Completeness, Goal Accuracy, Knowledge Retention, Role Adherence, Turn Relevancy.
  - **Never pick these**, whatever the app does — this file cannot carry what they need, so they always fail: Tool Correctness and Argument Correctness (need the tools the app called, which nothing here supplies); Misuse, Non-Advice, Pattern Match, Prompt Alignment, Role Violation, Topic Adherence (each needs a configuration parameter `metricSettings` has no field for); Turn Contextual Precision, Turn Contextual Recall, Turn Contextual Relevancy, Turn Faithfulness (need retrieval context, which a multi-turn golden cannot carry).
  - Rough fit, once the above filters are applied: RAG → Faithfulness / Answer Relevancy / Contextual\* (and give every golden a `retrievalContext`); agents → Answer Relevancy plus one trace-only metric; chatbots → the multi-turn list.
- **AI connection** — **always include this section when the repo exposes an HTTP route that serves the LLM/agent path** (a FastAPI/Flask/Django/Express/Next.js route handler that takes a question and returns an answer). A missing deployment URL is **not** a reason to skip it: the record is a starter config the user finishes in the UI, so a section with `name`, `payload` and `actualOutputJSONKeyPath` and no `endpoint` is correct and useful. Only omit the whole section when the repo has no such route at all (a CLI, a library, a batch job).
  ...
  - `endpoint` — the full URL evaluation requests are sent to. Build it in two halves and **join them**: the **route path** comes from the handler in code (`@app.post("/chat")` → `/chat`), and the **host** from wherever the repo states a deployed base URL — a README deployment section, a base-URL env var in `.env.example`, `vercel.json` / `railway.json` / `fly.toml` / `render.yaml`, a Dockerfile or k8s manifest, or CI config. A stated base URL plus a route path **is** a determinable endpoint — compose it rather than omitting (`https://api.example-app.com` + `/chat` → `https://api.example-app.com/chat`). Never invent a host, and treat an obvious stand-in as no host at all (`example.com`, `your-app.com`, `<your-domain>`, `changeme`). Only when no host appears anywhere, omit `endpoint` and keep the rest of the section. The result must be `https` with a publicly resolvable host; anything else is dropped server-side.
